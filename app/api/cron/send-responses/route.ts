...
export async function GET(req: NextRequest) {
  ...
  const todayStr = todayCat.toISOString().slice(0, 10);
  ...
  for (const doc of queuedDocs) {
    ...
    try {
      ...
      const subject = "Update on Your LaunchPadX Application";
...
function buildResponseEmail({ firstName, response }: { firstName: string; response: string }): string {
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /><title>LaunchPadX Application Update</title></head>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
        ...
        <tr><td style="padding:40px 40px 32px;">
          <p style="margin:0 0 8px;font-size:15px;color:#374151;">Dear ${firstName},</p>
          <p style="margin:0 0 24px;font-size:15px;color:#374151;line-height:1.6;">Thank you for taking the time to apply to LaunchPadX. After careful review of your application, we have an update for you.</p>
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#F9FAFB;border-left:4px solid #16A34A;border-radius:6px;margin-bottom:24px;">
            <tr><td style="padding:20px 24px;font-size:15px;color:#1F2937;line-height:1.7;white-space:pre-wrap;">${escapeHtml(response)}</td></tr>
          </table>
          <p style="margin:0 0 16px;font-size:14px;color:#6B7280;line-height:1.6;">If you have any questions, please reply to this email.</p>
